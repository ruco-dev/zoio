...
interface Options { mode?: "fixture" | "codex" | "openai"; model?: string; outputDir?: string; target?: string; depth?: number; maxQueries?: number; codexTimeoutMs?: number }
...
async function explore(location: string, business: string, options: Options): Promise<void> {
  ...
  while (queue.length && completed < max) { const item = queue.shift()!; const result = await scan(location, business, config, undefined, item.depth, item.parentId, item.query, undefined, (message) => console.error(`zoio: ${message}`)); if (seen.has(result.query.query)) continue; seen.add(result.query.query); completed += 1; print(result);
