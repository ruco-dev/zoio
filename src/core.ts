...
export async function scan(location: string, business: string, config: ZoioConfig, target?: string, depth = 0, parentId: string | null = null, query?: string, runId = `run_${randomUUID()}`): Promise<ScanResult> {
  const record = makeQuery(runId, location, business, depth, parentId, query); const search = await providerFor(config.mode).search(record.query, config.model);
