...
test("Codex provider uses an ephemeral JSONL session and keeps every event as raw provenance", async () => {
  const executable = await fakeCodex(`if (!process.argv.includes("--ephemeral")) { process.stderr.write("missing ephemeral mode"); process.exit(2); } console.log(JSON.stringify({ type: "thread.started", thread_id: "t1" })); console.log(JSON.stringify({ type: "item.completed", item: { type: "agent_message", text: "1. Hotel Azul — central\\nhttps://hotel-azul.example" } }));`);
